...
(defn mk-executor-data [worker executor-id]
  (let [worker-context (worker-context worker)
        ...
        ;;line added by uniroma2
        monitor-client (new MonitorClient  (.toString executor-id) (.toString (:storm-id worker)))
        ]
    (recursive-map
...
(defmethod mk-threads :spout [executor-data task-datas]
  (let [{:keys [storm-conf component-id worker-context transfer-fn report-error sampler open-or-prepare-was-called?]} executor-data
        ...
        tuple-action-fn (fn [task-id ^TupleImpl tuple]
                          (.updateThreadId (:monitor-client executor-data))
                            ...
                            (condp = stream-id
                              ...
                              (let [task-data (get task-datas task-id)
                                ...
                                ;; Uniroma2 -- end
                                     ...
                                     (when (not (nil? (:monitor-client  executor-data)))
                                       (.AdvertiseTuplereceivedClojure (:monitor-client executor-data) tuple))
                                     ;; end section 
    ...
    [(async-loop
      (fn []
        ...
        (doseq [[task-id task-data] task-datas
          ...
          (.setMc user-context (:monitor-client executor-data))
          (.prepare bolt-obj
                    storm-conf
                    user-context
                    ;; Uniroma2: 
