...
public class Client implements IConnection {
    ...
    Client(Map storm_conf, ChannelFactory factory, ScheduledExecutorService scheduler, String host, int port) {
	...
	remHostName(remote_addr.getHostName());
	if (mm == null) {
	    ...
	    Long fetchtime = ((Integer) confs.get(Config.TAOS_SCHEDULE_FETCH_TIME)).longValue();
	    mm = new MongoManager(mongoIp, mongoPort, ttl, updateDamping.floatValue());

	}
	isValid = false;
	PlacementResponse pr = mm.getDepl();
	for (Assignment ass : pr.getAssignment()) {
	    String hostname = pr.getHostToNodeId().get(ass.getNodeId());
	    int port = ass.getPort();
	    if (name().contains(port + "") && name().contains(hostname) && !ass.getExecutorsIds().isEmpty()) {
		isValid = true;
	    }
	}
	if (!isValid) {
	...
	try {
	    ...
	    if (null != channel) {
		addHostName(remote_addr.getHostName());
	...
	if (!closing) {
	    ...
	    final long timeoutMilliSeconds = 60 * 1000; // 600 seconds
	    final long start = System.currentTimeMillis();
	    if (!failure) {
		LOG.info("Waiting for pending batchs to be sent with " + name() + "..., timeout: {}ms, pendings: {}", timeoutMilliSeconds,
			pendings.get());
		...
		while (pendings.get() != 0) {
			...
			if (delta > timeoutMilliSeconds) {
			    LOG.error("Timeout when sending pending batchs with {}..., there are still {} pending batchs not sent", name(),
				    pendings.get());
			    break;
			}
    ...
    private static void addHostName(String hostname) {
	synchronized (HOSTNAMES) {
	    HOSTNAMES.add(hostname);
	}
    }

    private static void remHostName(String hostname) {
	synchronized (HOSTNAMES) {
	    while (HOSTNAMES.remove(hostname)) {
	    }
	}
