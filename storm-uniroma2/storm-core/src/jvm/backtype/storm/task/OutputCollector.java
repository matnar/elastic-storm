...
/**
 * This output collector exposes the API for emitting tuples from an IRichBolt. This is the core API for emitting tuples. For a simpler API, and a more restricted form of stream processing, see
 * IBasicBolt and BasicOutputCollector.
 */
public class OutputCollector implements IOutputCollector {
    ...
    /**
     * Emits a new unanchored tuple to the specified stream. Because it's unanchored, if a failure happens downstream, this new tuple won't affect whether any spout tuples are considered failed or
     * not. The emitted values must be immutable.
     * 
     * @param streamId
     *            the stream to emit to
     * @param tuple
    ...
    /**
     * Emits a new unanchored tuple to the default stream. Beacuse it's unanchored, if a failure happens downstream, this new tuple won't affect whether any spout tuples are considered failed or not.
    ...
    /**
     * Emits a tuple directly to the specified task id on the specified stream. If the target bolt does not subscribe to this bolt using a direct grouping, the tuple will not be sent. If the specified
     * output stream is not declared as direct, or the target bolt subscribes with a non-direct grouping, an error will occur at runtime. The emitted values must be immutable.
     *
     * @param taskId
     *            the taskId to send the new tuple to
     * @param streamId
     *            the stream to send the tuple on. It must be declared as a direct stream in the topology definition.
     * @param anchor
     *            the tuple to anchor to
     * @param tuple
     *            the new output tuple from this bolt
    ...
    /**
     * Emits a tuple directly to the specified task id on the specified stream. If the target bolt does not subscribe to this bolt using a direct grouping, the tuple will not be sent. If the specified
     * output stream is not declared as direct, or the target bolt subscribes with a non-direct grouping, an error will occur at runtime. Note that this method does not use anchors, so downstream
     * failures won't affect the failure status of any spout tuples. The emitted values must be immutable.
     *
     * @param taskId
     *            the taskId to send the new tuple to
     * @param streamId
    ...
    /**
     * Emits a tuple directly to the specified task id on the default stream. If the target bolt does not subscribe to this bolt using a direct grouping, the tuple will not be sent. If the specified
     * output stream is not declared as direct, or the target bolt subscribes with a non-direct grouping, an error will occur at runtime. The emitted values must be immutable.
     *
     * <p>
